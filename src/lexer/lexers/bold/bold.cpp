...
namespace mark_sideways
{
    ...
    namespace lexers
    {
        ...
        mark_sideways::lexers::Bold::Bold(mark_sideways::State *state) : AbstractLexer(state)
        {
        ...
        std::vector<mark_sideways::Token> mark_sideways::lexers::Bold::lex(const std::string &line)
        {
            ...
            else
            {
                ...
                else if (std::regex_search(line, match, this->match_regex))
                {
                    ...
                    tokens.push_back(mark_sideways::Token(mark_sideways::Token::token_type::BOLD, match.str(1)));
                    if (after_tokens.size() > 0)
                    {
                        tokens.insert(tokens.end(), after_tokens.begin(), after_tokens.end());
                    }
