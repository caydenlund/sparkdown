...
namespace sparkdown
{
    ...
    namespace lexers
    {
        ...
        sparkdown::lexers::Italic::Italic(sparkdown::State *state) : AbstractLexer(state)
        {
        ...
        std::vector<sparkdown::Token> sparkdown::lexers::Italic::lex(const std::string &line)
        {
            ...
            else
            {
                ...
                else if (std::regex_search(line, match, this->match_regex))
                {
                    ...
                    tokens.push_back(sparkdown::Token(sparkdown::Token::token_type::ITALIC, match.str(1)));
                    if (after_tokens.size() > 0)
                    {
                        tokens.insert(tokens.end(), after_tokens.begin(), after_tokens.end());
                    }
